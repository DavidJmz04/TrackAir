    ...
    async function (peticion, respuesta) {
      ...
      fetch(
        ...
        .then((data) => {
          ...
          // todo ok
          fetch(
            "https://webcat-web.gva.es/webcat_web/datosOnlineRvvcca/obtenerTablaPestanyaDatosOnline",
            options
          )
            .then((response) => response.text())
            .then((data) => {
              ...
              // todo ok
              respuesta.send(
                utilidad.convertirTiempoRealAJSONpropio(
                  JSON.parse(data)["listMagnitudesMediasHorarias"]
                )
              );
            });
        });
