...
module.exports = class Utilidades {
    ...
    convertirTiempoRealAJSONpropio(csv) {
        ...
        i = 0
        sumas.forEach(suma => {
            lecturas += '{"contaminación":' + suma + ',"calidad":"' + calidadAire[this.procesarCalidadAire({
                "tipoMedicion": "O3"
            }, suma)] + '"}'
            total += suma
        ...
        text = '{"fecha":"' + this.fechaActual() + '", "calidadMedia":"' + calidadAire[this.procesarCalidadAire({
            "tipoMedicion": "O3"
        }, total)] + '", "mediciones":[' + lecturas + "]}"
        return JSON.parse(text)
    }
